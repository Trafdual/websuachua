                ...
                if (randomNumber < cumulativePercent) {
                    if (prizes[i].text === 'Other') {
                        const otherGifts = listGift.filter(
                            gift => gift.text !== 'Wave Alpha 110' && gift.text !== 'Iphone 13'
                        );
                        const randomOtherIndex = Math.floor(Math.random() * otherGifts.length);
                        return { ...otherGifts[randomOtherIndex], index: listGift.indexOf(otherGifts[randomOtherIndex]) };
                    } else {
                        const selectedIndex = listGift.findIndex(gift => gift.text === prizes[i].text);
                        return { ...listGift[selectedIndex], index: selectedIndex };
                    }
                }
            }
        } else {
            const fixedGiftIndex = listGift.findIndex(gift => gift.text === 'Tai nghe không dây');
